...
export class AccountingService {
  ...
  async findAllAccounts(
    ...
    if (codes) {
      ...
      if (codeList.length > 0) {
        const conditions = codeList.map(
          (c, i) => `account.code LIKE :code_${i}`,
        );
        const params = codeList.reduce(
          (acc, c, i) => {
            acc[`code_${i}`] = `${c}%`;
            return acc;
          },
  ...
  async findAccountTree(): Promise<AccountingAccount[]> {
    ...
    const map = new Map<
      number,
      AccountingAccount & { children: AccountingAccount[] }
    >();
  ...
  async getExchangeRate(
    ...
    throw new BadRequestException(
      `No exchange rate found from ${from} to ${to} on or before ${formattedDate}`,
    );
  }
  ...
  async createJournalEntry(dto: CreateJournalEntryDto): Promise<JournalEntry> {
    ...
    for (const lineDto of dto.lines) {
      ...
      if (!account) {
        throw new NotFoundException(
          `Account with ID ${lineDto.accountId} not found`,
        );
      }
    ...
    if (diff > 0 && diff <= 0.02 && lines.length > 0) {
      ...
      const bal = Number(
        ((lastLine.debit - lastLine.credit) * lastLine.exchangeRate).toFixed(2),
      );
