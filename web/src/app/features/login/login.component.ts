...
@Component({
  ...
  imports: [FormsModule],
  template: `
    <div class="min-h-screen flex flex-col justify-center items-center bg-zinc-950 px-4">
      <div class="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl p-8 shadow-2xl space-y-8">

        <!-- Logo Header -->
        <div class="flex flex-col items-center space-y-3">
          <svg width="224" height="48" viewBox="0 0 224 48" fill="none" xmlns="http://www.w3.org/2000/svg" class="h-10 w-auto">
          ...
          <div class="text-center">
            <h2 class="text-xl font-bold text-white tracking-wide">Operations Hub</h2>
            <p class="text-zinc-500 text-xs mt-1">Please sign in to continue</p>
          </div>
        </div>

        <!-- Error Alert -->
        @if (error()) {
          <div class="bg-red-900/30 border border-red-800 text-red-300 px-4 py-3 rounded-lg text-sm text-center">
            {{ error() }}
          </div>
        }

        <!-- Form -->
        <form #loginForm="ngForm" (ngSubmit)="loginForm.valid && submit()" class="space-y-5">
          <!-- Email -->
          <div>
            ...
            <input
              ...
              class="w-full bg-zinc-950 border rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-zinc-500 placeholder-zinc-600"
              [class.border-zinc-800]="!emailInput.invalid || (!emailInput.dirty && !emailInput.touched)"
              [class.border-red-500]="emailInput.invalid && (emailInput.dirty || emailInput.touched)"
              placeholder="name@kora.com"
              autofocus
            />
            @if (emailInput.invalid && (emailInput.dirty || emailInput.touched)) {
              <p class="text-red-500 text-xs mt-1.5">
                @if (emailInput.errors?.['required']) { Email is required }
                @else { Must be a valid email }
              </p>
            }
          </div>
          ...
          <div>
            ...
            <input
              type="password"
              [(ngModel)]="password" name="password" required #passwordInput="ngModel"
              class="w-full bg-zinc-950 border rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-zinc-500 placeholder-zinc-600"
              [class.border-zinc-800]="!passwordInput.invalid || (!passwordInput.dirty && !passwordInput.touched)"
          ...
          <button
            ...
            class="w-full py-2.5 bg-white text-zinc-900 text-sm font-semibold rounded-lg hover:bg-zinc-100 transition-colors disabled:opacity-50 cursor-pointer flex justify-center items-center gap-2"
          >
            @if (loading()) {
              <span class="w-4 h-4 border-2 border-zinc-900 border-t-transparent rounded-full animate-spin"></span>
              Signing in...
            } @else {
              Sign In
            }
